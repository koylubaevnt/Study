...
// Promise, который сразу переходит в состояние resolved с данными массива phrases
let phrasesPromise = Promise.resolve(phrases);

// Сервис для работы с даннными
// в будущем его можно переделать на работу с сервером
@Injectable()
export class PhraseService {

    // Метод для получения всех фраз. Возвращает Promise с массивлм Phrase
    getAll(): Promise<Phrase[]> {
        return phrasesPromise;
    }

    // Метод для получения фразы по id. Возвращает Promise с экземплятор Phrase
